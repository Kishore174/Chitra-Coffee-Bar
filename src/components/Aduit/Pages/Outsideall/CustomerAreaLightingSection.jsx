...
const CustomerAreaLightingSection = () => {
  const [lightingImagePreview, setLightingImagePreview] = useState([]);
  const [isLightingSubmitted, setIsLightingSubmitted] = useState(false);
  const [previewLightingImage, setPreviewLightingImage] = useState(null);
  const lightingFileInputRef = useRef();
  ...
  const handleLightingClick = (image) => {
    setPreviewLightingImage(image);
  };

  const handleCloseLighting = () => {
    setPreviewLightingImage(null);
  };

  const triggerLightingFileInput = () => {
    lightingFileInputRef.current.click();
  };

  const handleLightingSubmit = () => {
    setIsLightingSubmitted(true);
  ...
  const handleLightingRemarkClick = (remark) => {
    setSelectedLightingRemark(remark);
  };
  ...
  return (
    <div className="p-4 bg-white relative flex flex-col justify-between shadow-md rounded-md mb-4">
      ...
      <label className="text-sm font-medium text-gray-500 mb-2 block">Hygiene</label>
        <div className="flex gap-2">
          {['Good', 'Bad'].map((remark) => (
            <div
              key={remark}
              onClick={() => handleLightingRemarkClick(remark)}
              className={`cursor-pointer px-4 py-2 rounded-full border flex items-center justify-center transition-colors duration-200 
                hover:bg-red-600 hover:text-white ${selectedLightingRemark === remark ? 'bg-red-600 text-white' : 'bg-white text-gray-700 border-gray-300'}`}
            >
              {remark}
            </div>
          ))}
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {lightingImagePreview.map((image, index) => (
          <div key={index} className="relative">
            <img
              src={image}
              alt={`Lighting ${index + 1}`}
              className="h-12 w-12 border rounded-md object-cover cursor-pointer"
              onClick={() => handleLightingClick(image)}
            />
            <button
              onClick={() => removeLightingImage(index)}
        ...
        <div
          onClick={triggerLightingFileInput}
        ...
        <input
          ...
          onChange={handleLightingPhotoCapture}
          className="hidden"
          multiple
        />
      </div>
      ...
      {previewLightingImage && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="relative bg-white p-4 rounded-lg">
            <img src={previewLightingImage} alt="Preview" className="max-h-96 max-w-full rounded" />
            <button
              onClick={handleCloseLighting}
...
export default CustomerAreaLightingSection;
