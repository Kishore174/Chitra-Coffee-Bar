...
const HandWashSection = () => {
  const [handWashImagePreview, setHandWashImagePreview] = useState([]);
  const [isHandWashSubmitted, setIsHandWashSubmitted] = useState(false);
  const [previewHandWashImage, setPreviewHandWashImage] = useState(null);
  const handWashFileInputRef = useRef();
  const [rating, setRating] = useState(0); // State for storing rating
  const [selectedHandWashRemark, setSelectedHandWashRemark] = useState(null); // State for selected remark
  ...
  const handleHandWashPhotoCapture = async(e) => {
    ...
    files.forEach((file) => {
      ...
      reader.onloadend = () => {
        ...
        img.onload = () => {
          ...
          setHandWashImagePreview((prev) => [...prev, watermarkedImage]);
        };
      };
  ...
  const removeHandWashImage = (index) => {
    setHandWashImagePreview((prev) => prev.filter((_, i) => i !== index));
  };
  ...
  const handleCloseHandWash = () => {
    setPreviewHandWashImage(null);
  };

  const triggerHandWashFileInput = () => {
    handWashFileInputRef.current.click();
  };

  const handleHandWashSubmit = () => {
    setIsHandWashSubmitted(true);
  ...
  const handleHandWashRemarkClick = (remark) => {
    setSelectedHandWashRemark(remark);
  };

  return (
    <div className="p-4 bg-white flex flex-col justify-between relative shadow-md rounded-md mb-4">
    <h2 className="text-lg font-semibold  text-center mb-2">Hand Wash Section</h2>
    ...
    <label className="text-sm font-medium text-gray-500 mb-2 block">Hygiene</label>
      ...
      <div className="flex gap-2">
        {['Good', 'Bad'].map((remark) => (
          <div
            key={remark}
            onClick={() => handleHandWashRemarkClick(remark)}
            className={`cursor-pointer px-4 py-2 rounded-full border flex items-center justify-center transition-colors duration-200 
              hover:bg-red-600 hover:text-white ${selectedHandWashRemark === remark ? 'bg-red-600 text-white' : 'bg-white text-gray-700 border-gray-300'}`}
          >
            {remark}
          </div>
        ))}
    ...
    {/* Rating Section */}
    <div className="mb-4">
      <label className="text-sm font-semibold text-gray-700">Rate your hand wash experience</label> {/* Added label */}
      <div className="flex">
        {[1, 2, 3, 4, 5].map((star) => (
          <StarIcon
            key={star}
    ...
    <div className="flex flex-wrap gap-2 mb-4">
      {handWashImagePreview.map((image, index) => (
        <div key={index} className="relative">
          ...
          />
          <button
            onClick={() => removeHandWashImage(index)}
      ...
      <div
        onClick={triggerHandWashFileInput}
      ...
      <input
        ...
        onChange={handleHandWashPhotoCapture}
        className="hidden"
        multiple
      />
    </div>
    ...
    {/* <button
      onClick={handleHandWashSubmit}
      className={`mt-4 w-full text-center mx-auto py-2 rounded-md text-white ${
        isHandWashSubmitted ? 'bg-green-600' : 'bg-red-600 hover:bg-red-700'
      }`}
    >
      {isHandWashSubmitted ? (
   ...
   */}
    {previewHandWashImage && (
      <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
        <div className="relative bg-white p-4 rounded-lg">
          <img src={previewHandWashImage} alt="Preview" className="max-h-96 max-w-full rounded" />
          <button
            onClick={handleCloseHandWash}
