...
const SetRoutes = () => {
  ...
  const flattenedShops = set.flat().map((shop) => shop);
    ...
    <div className="w-full">
      <div className="relative w-3/6 p-6">
        ...
        <div
          ...
          tabIndex={0}
          onKeyDown={(e) =>
            e.key === "Enter" && setDropdownOpen((prev) => !prev)
          }
        >
            ...
            <ul className="py-1">
              {filteredOptions.length > 0 ? (
                filteredOptions.map((option) => (
                  <li
                    key={option._id}
                    className={`p-3 cursor-pointer hover:bg-gray-200 transition duration-200 ${
