...
const LiveSnacks = () => {
  ...
  const handleLiveSnackSubmit = async(e) => {
    ...
    const formData = {
      ...
      remark,
      rating,
      liveSnackImagePreview,
    };
    const res = await createCoffee(auditId, formData);
  ...
  <div className="absolute left-0 mt-2 w-56 p-4 bg-white rounded-lg shadow-lg border border-gray-300 transition-transform transform duration-200 ease-in-out">
    ...
    <form onSubmit={handleLiveSnackSubmit} className="w-full max-w-4xl mx-auto p-8 ">
        <div className="bg-white p-6 rounded-lg shadow-md mb-8">
          ...
          {/* Snack Availability (Grid layout with 4 columns) */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            {['Bajji', 'Samosa', 'Bonda', 'Medhu Vadai'].map((snack) => (
              <div key={snack} className="flex flex-col text-left   ">
                <h2 className="text-lg font-semibold text-gray-800   mb-2">{snack}</h2>
                <div className="flex space-x-4 mb-2">
                  {['available', 'not available'].map((status) => (
                    <div
                      key={status}
                      onClick={() => {
                        if (snack === 'Bajji') setBajjiAvailable(status);
                        if (snack === 'Samosa') setSamosaAvailable(status);
                        if (snack === 'Bonda') setBondaAvailable(status);
                        if (snack === 'Medhu Vadai') setMedhuVadaiAvailable(status);
                      } }
                      className={`cursor-pointer capitalize px-3 py-1 font-medium rounded-full border transition-colors duration-200 hover:bg-red-600 hover:text-white ${(snack === 'Bajji' && bajjiAvailable === status) ||
                          (snack === 'Samosa' && samosaAvailable === status) ||
                          (snack === 'Bonda' && bondaAvailable === status) ||
                          (snack === 'Medhu Vadai' && medhuVadaiAvailable === status)
