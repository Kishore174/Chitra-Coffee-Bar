...
const BunzoSection = () => {
  ...
  const [isBunzoSubmitted, setIsBunzoSubmitted] = useState(false);
  const [previewBunzoImage, setPreviewBunzoImage] = useState(null);
  const bunzoFileInputRef = useRef();
  const [rating, setRating] = useState(0);
  const [remark, setRemark] = useState(''); // State for remarks
  const [isBunzoAvailable, setIsBunzoAvailable] = useState(null); // State for bunzo availability
  ...
  const handleBunzoPhotoCapture =async (e) => {
    ...
    files.forEach((file) => {
      ...
      reader.onloadend = () => {
        ...
        img.onload = () => {
          ...
          setBunzoImagePreview((prev) => [...prev, watermarkedImage]);
        };
      };
  ...
  const removeBunzoImage = (index) => {
    setBunzoImagePreview((prev) => prev.filter((_, i) => i !== index));
  };

  const handleBunzoClick = (image) => {
    setPreviewBunzoImage(image);
  };

  const handleCloseBunzo = () => {
    setPreviewBunzoImage(null);
  };
  ...
  const handleBunzoSubmit = () => {
    setIsBunzoSubmitted(true);
  ...
  return (
    <div className="p-4 bg-white relative shadow-md rounded-md mb-4">
      <h2 className="text-lg font-semibold mb-2">Bunzo Section</h2>

      {/* Availability Section */}
      <div className="mb-4">
        <h3 className="font-medium text-gray-700 text-sm">Is Bunzo available</h3>
      ...
      {/* Rating Section */}
      <div className="mb-4">
        <h3 className="font-medium text-gray-700 text-sm">Rate your Bunzo experience</h3>
        <div className="flex">
          {[1, 2, 3, 4, 5].map((star) => (
            <StarIcon
              key={star}
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {bunzoImagePreview.map((image, index) => (
          <div key={index} className="relative">
            <img
              src={image}
              alt={`Bunzo ${index + 1}`}
              className="h-12 w-12 border rounded-md object-cover cursor-pointer"
              onClick={() => handleBunzoClick(image)}
            />
            <button
              onClick={() => removeBunzoImage(index)}
      ...
      {/* <button
        onClick={handleBunzoSubmit}
        className={`mt-4 w-full text-center mx-auto py-2 rounded-md text-white ${
          isBunzoSubmitted ? 'bg-green-600' : 'bg-red-600 hover:bg-red-700'
        }`}
      >
        {isBunzoSubmitted ? (
      ...
      {previewBunzoImage && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="relative bg-white p-4 rounded-lg">
            <img src={previewBunzoImage} alt="Preview" className="max-h-96 max-w-full rounded" />
            <button
              onClick={handleCloseBunzo}
