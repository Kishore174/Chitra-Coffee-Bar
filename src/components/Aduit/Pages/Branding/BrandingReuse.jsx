...
const BrandingReuse = ({ title, itemType, onUpdate, data }) => {
    ...
    const [pillarCount, setPillarCount] = useState(''); // State for the brand name
    ...
    useEffect(() => {
      if (data) {
        setPillarCount(data.pillarCount);
    ...
    { available !== "no" &&  <>
      {itemType === "pillarBranding" && (
        <div> 
          ...
          <input
            type="number"
            placeholder="Pillar Count"
            value={pillarCount} // Bind the input value to brandName state
            onChange={(e) => setPillarCount(e.target.value)} // Update brandName state on change
